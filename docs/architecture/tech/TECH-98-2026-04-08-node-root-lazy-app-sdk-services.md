> Migrated from `docs/架构/98-2026-04-08-node-root-lazy-app-sdk-services.md` on 2026-06-24.
...
- `sdkwork-agentstudio-pc-community` and `sdkwork-agentstudio-pc-dashboard` both consume these services through the package root of `@sdkwork/agentstudio-pc-core`.
...
- Feature packages must keep consuming `@sdkwork/agentstudio-pc-core` through the package root instead of reaching into package-internal paths to work around Node-safe gaps.

## Impact

- `sdkwork-agentstudio-pc-community` can import `communityService` and `createCommunityService` in Node-safe tests without crashing during module evaluation.
- `sdkwork-agentstudio-pc-dashboard` can import `dashboardCommerceService` and `createEmptyDashboardCommerceSnapshot` through the Node root while still letting tests override commerce APIs explicitly.
