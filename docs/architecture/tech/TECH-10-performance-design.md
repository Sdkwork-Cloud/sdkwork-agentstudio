...
- Frozen truth chain:
  ...
  - `packages/sdkwork-agentstudio-pc-infrastructure/src/platform/contracts/runtime.ts` freezes those fields in `RuntimeDesktopLocalAiProxyInfo`
  - `packages/sdkwork-agentstudio-pc-settings/src/services/kernelCenterService.ts` forwards them unchanged into `KernelCenterDashboard.localAiProxy`
  - `packages/sdkwork-agentstudio-pc-settings/src/KernelCenter.tsx` formats readback only
...
- Frozen truth chain:
  ...
  - `packages/sdkwork-agentstudio-pc-settings/src/services/localAiProxyLogsService.ts` now exposes `getRuntimeSummary()` by reading `kernelPlatformService.getInfo()?.localAiProxy`
  - `packages/sdkwork-agentstudio-pc-settings/src/ApiSettings.tsx` renders readback only
- UI obligations in this slice:
