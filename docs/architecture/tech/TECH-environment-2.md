> Migrated from `docs/zh-CN/reference/environment.md` on 2026-06-24.
...
| `CLAW_SERVER_CONFIG` | 可选 | `agentstudio-server` 使用的 JSON 配置文件路径 |
...
| `SDKWORK_SERVER_BUILD_WSL_DISTRO` | 可选 | 在 Windows 上为 `pnpm build:server -- --target <linux-triple>` 指定自动桥接使用的 WSL 发行版 |
...
- `agentstudio-server` 当前的配置优先级是 `CLI 覆盖 -> 配置文件 -> 环境变量 -> 默认值`。
- `agentstudio-server service print-manifest` 会复用同一套解析优先级；如果没有显式配置文件路径，则会默认落到 `<CLAW_SERVER_DATA_DIR>/agentstudio-server.config.json`。
- `agentstudio-server service install`、`start`、`stop`、`restart` 与 `status` 也会复用同一套配置入口，并要求操作者具备平台服务管理器所需的权限。
...
- 在 Windows 上，`pnpm build:server -- --target x86_64-unknown-linux-gnu` 会优先复用已安装的 WSL 发行版来完成 Linux release 构建；如果需要固定发行版，可设置 `SDKWORK_SERVER_BUILD_WSL_DISTRO`，如果需要关闭这条桥接路径，可设置 `SDKWORK_SERVER_BUILD_DISABLE_WSL=1`。
