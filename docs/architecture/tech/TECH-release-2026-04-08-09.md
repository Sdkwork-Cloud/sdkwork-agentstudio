...
- Step 03 continued with an instance-workbench contract-alignment loop that refreshes the largest remaining `sdkwork-clawstudio-instances` test file onto the current managed-config, channel-definition, and workbench-task contracts.
- No production behavior changed in this loop; the work is strictly test-contract and helper-layer repair.

## Attempt Outcome

- `packages/sdkwork-clawstudio-instances/src/services/instanceWorkbenchService.test.ts` now uses typed helper constructors for managed OpenClaw snapshots, config routes, channel definitions, and live cron-task fixtures.
...
- The workspace lint frontier has advanced to the next `sdkwork-clawstudio-instances` test files headed by `openClawManagementCapabilities.test.ts`.
