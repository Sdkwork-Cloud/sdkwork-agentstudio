> Migrated from `docs/架构/131-2026-04-08-desktop-startup-evidence-descriptor-endpoint-owner.md` on 2026-06-24.
> Owner: SDKWork maintainers

# 131-2026-04-08 Desktop Startup Evidence Descriptor Endpoint Owner

## Decision

If persisted desktop startup evidence already contains shell-relevant startup descriptor endpoint fields, the desktop kernel/runtime publication chain must preserve them instead of truncating them before shell consumption.
...
Settings must not recover these descriptor endpoint facts by reading `diagnostics/desktop-startup-evidence.json` directly or by inferring them from the current live runtime contract.

## Why

- `descriptorEndpointId` is the startup-time hosted runtime endpoint identity surface. Without it, the shell cannot tell which endpoint the persisted startup snapshot validated.
- `descriptorActivePort` is the startup-time hosted runtime published port surface. Without it, the shell cannot tell which concrete active port the startup descriptor had reached.
...
- `Kernel Center` may present the published startup descriptor endpoint fields, but it may not:
  ...
  - reconstruct startup endpoint facts from the current live host state

## Impact

- `CP03-4` now has an explicit rule for shell-facing startup-evidence descriptor endpoint truth.
