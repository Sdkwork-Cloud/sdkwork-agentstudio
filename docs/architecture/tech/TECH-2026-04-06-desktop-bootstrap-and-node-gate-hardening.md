...
2. `pnpm.cmd check:desktop` was not actually stable because its TypeScript test execution path could not resolve workspace packages such as `@sdkwork/agentstudio-pc-infrastructure` when Node ran `.ts` files directly.
...
- the direct Node execution path had no workspace alias resolution for local `@sdkwork/agentstudio-pc-*` packages
- `desktopHostedBridge.test.ts` therefore failed to import `@sdkwork/agentstudio-pc-infrastructure`
...
The desktop `vite.config.ts` was using `__dirname` without the `fileURLToPath(import.meta.url)` bridge that already exists in `packages/sdkwork-agentstudio-pc-web/vite.config.ts`.
...
- added regression coverage for `@sdkwork/agentstudio-pc-infrastructure` workspace alias resolution
...
- `node --experimental-transform-types packages/sdkwork-agentstudio-pc-desktop/src/desktop/bootstrap/DesktopBootstrapApp.test.ts`
- `node scripts/desktop-hosted-runtime-regression-contract.test.mjs`
- `pnpm.cmd --filter @sdkwork/agentstudio-pc-desktop build`
