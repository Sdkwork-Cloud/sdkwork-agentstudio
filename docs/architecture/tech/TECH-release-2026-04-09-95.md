...
- Implemented and closed the unfinished Step 07 extraction loop:
  - added `packages/sdkwork-clawstudio-instances/src/services/openClawManagedChannelMutationSupport.ts`
  - rewired `packages/sdkwork-clawstudio-instances/src/pages/InstanceDetail.tsx` so the page now builds managed-channel draft and mutation requests through the dedicated helper and runs shared orchestration through injected page-owned callbacks
  - added `packages/sdkwork-clawstudio-instances/src/services/openClawManagedChannelMutationSupport.test.ts`
