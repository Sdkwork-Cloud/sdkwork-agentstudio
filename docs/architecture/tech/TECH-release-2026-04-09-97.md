...
# Agent Studio release-2026-04-09-97
...
- Implemented the Step 07 extraction:
  - added `packages/sdkwork-agentstudio-pc-instances/src/services/openClawManagedConfigMutationSupport.ts`
  - rewired `packages/sdkwork-agentstudio-pc-instances/src/pages/InstanceDetail.tsx` so the page now builds one injected managed-config runner from the shared helper and removes the inline save-orchestration cluster
  - added `packages/sdkwork-agentstudio-pc-instances/src/services/openClawManagedConfigMutationSupport.test.ts`
