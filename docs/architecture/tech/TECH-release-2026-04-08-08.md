...
- `packages/sdkwork-clawstudio-chat/src/services/openClawGatewayHistoryConfigService.test.ts` now matches the live `StudioInstanceDetailRecord` contract used by `webStudio.ts` and the current `OpenClawConfigSnapshot` shape exposed by `@sdkwork/clawstudio-core`.
...
- The workspace lint frontier has advanced to `packages/sdkwork-clawstudio-instances/src/services/instanceWorkbenchService.test.ts`.
...
- `node --experimental-strip-types packages/sdkwork-clawstudio-chat/src/services/openClawGatewayHistoryConfigService.test.ts`
