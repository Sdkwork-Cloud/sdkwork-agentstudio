> Migrated from `docs/review/2026-06-06-sdkwork-http-boundary-classification.md` on 2026-06-24.
> Owner: SDKWork maintainers

Date: 2026-06-06

Scope: Agent Studio raw transport, manual auth-header, and retired SDK references after the SDKWork appbase, Drive, messaging, local-router, and IAM runtime realignment.
...
Appbase login, registration, session refresh, logout, OAuth, QR auth, password reset, and verification-code flows are no longer classified as raw HTTP. `packages/sdkwork-agentstudio-pc-core/src/services/appAuthService.ts` delegates to `@sdkwork/auth-pc-react/auth-service`, `@sdkwork/iam-app-sdk`, and `@sdkwork/messaging-app-sdk`.

## Fixed SDKWork API Bypasses

`packages/sdkwork-agentstudio-pc-core/src/services/appAuthService.ts`
...
The following services still represent Claw product/business app-api capability. They are not backed by an identified Claw-owned SDK family under `agent-studio/sdks/`, and no current sibling repository exposes these product resources as a generated dependency SDK. They therefore remain an unresolved product SDK authority gap, not an appbase capability.

`packages/sdkwork-agentstudio-pc-core/src/sdk/appSdkPort.ts`
...
- Current status: uses injected structural feed/comment/category client ports through `getagentstudioAppClientWithSession`.
- Rule: do not add raw HTTP for feed, comment, or category resources.

`packages/sdkwork-agentstudio-pc-core/src/services/accountService.ts`

- Classification: unresolved product SDK authority.
- Current status: uses injected structural account client ports through `getagentstudioAppClientWithSession`.
- Rule: do not replace account summary, cash, recharge, or withdraw calls with raw HTTP.

`packages/sdkwork-agentstudio-pc-core/src/services/settingsService.ts`
...
- Classification: unresolved product SDK authority.
- Current status: uses injected structural skill client ports through `getagentstudioAppClientWithSession`.
...
- Classification: unresolved product SDK authority.
- Current status: uses injected structural feedback and FAQ client ports through `getagentstudioAppClientWithSession`.
...
- Classification: unresolved product SDK authority.
- Current status: uses injected structural order and product client ports through `getagentstudioAppClientWithSession`.
...
`packages/sdkwork-agentstudio-pc-core/src/lib/llmService.ts`
...
`packages/sdkwork-agentstudio-pc-infrastructure/src/platform/web.ts`
...
`packages/sdkwork-agentstudio-pc-infrastructure/src/http/httpClient.ts`
...
- `packages/sdkwork-agentstudio-pc-core/src/services/accountService.test.ts`
...
- `packages/sdkwork-agentstudio-pc-infrastructure/src/updates/updateClient.test.ts`
...
Before creating `agent-studio/sdks/`, identify the owning API authority and route manifests for the product resources above. The closure path must be:
