...
- Root cause:
  - `packages/sdkwork-clawstudio-desktop/src-tauri/src/framework/services/local_ai_proxy.rs` still mixed request-serving with Anthropic/Gemini/Ollama response text extraction, stop-reason mapping, OpenAI chat-completion shaping, OpenAI responses-api shaping, and embeddings response shaping in the same runtime hotspot.
