> Migrated from `docs/架构/132-2026-04-08-desktop-startup-evidence-descriptor-requested-port-owner.md` on 2026-06-24.
> Owner: SDKWork maintainers

# 132-2026-04-08 Desktop Startup Evidence Descriptor Requested Port Owner

## Decision

If persisted desktop startup evidence already contains shell-relevant startup descriptor port-allocation fields, the desktop kernel/runtime publication chain must preserve them instead of truncating them before shell consumption.

For the current Step 03 `CP03-4` convergence scope, that explicitly includes:

- `descriptorRequestedPort`
- `descriptorDynamicPort`
...
Settings must not recover these descriptor port-allocation facts by reading `diagnostics/desktop-startup-evidence.json` directly or by inferring them from the current live runtime contract.

## Why

- `descriptorRequestedPort` is the startup-time hosted runtime requested bind-port surface. Without it, the shell cannot tell which operator-intended port the persisted startup snapshot was validating.
- `descriptorDynamicPort` is the startup-time hosted runtime fallback/allocation surface. Without it, the shell cannot tell whether the startup descriptor succeeded on the requested port or had already moved into a dynamic-port path.
...
- `Kernel Center` may present the published startup descriptor port-allocation fields, but it may not:
  ...
  - reconstruct startup port-allocation facts from the current live host state

## Impact

- `CP03-4` now has an explicit rule for shell-facing startup-evidence descriptor requested-port and dynamic-port truth.
