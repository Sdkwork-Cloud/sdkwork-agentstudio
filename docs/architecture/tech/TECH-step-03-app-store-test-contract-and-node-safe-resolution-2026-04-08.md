> Migrated from `docs/review/step-03-app-store-test-contract-and-node-safe-resolution-2026-04-08.md` on 2026-06-24.
...
- After the previous Step 03 provider/agent loop, the workspace lint frontier moved to `packages/sdkwork-agentstudio-pc-apps/src/services/appStoreService.test.ts`.
...
- The runtime failure was not another missing `@sdkwork/agentstudio-pc-core` Node export.
  `packages/sdkwork-agentstudio-pc-apps/src/services/appStoreService.ts` was statically importing the value export `appStoreCatalogService` from `@sdkwork/agentstudio-pc-core`, but the Node-safe root entry intentionally does not expose that value.
...
- `packages/sdkwork-agentstudio-pc-apps/src/services/appStoreService.ts`
  - Replaced the eager default value import with lazy `import('@sdkwork/agentstudio-pc-core')` resolution.
...
| Command | Result | Note |
| --- | --- | --- |
| `node --experimental-strip-types packages/sdkwork-agentstudio-pc-apps/src/services/appStoreService.test.ts` | passed | 18 app-store regression cases stayed green after the test-contract and lazy-resolution updates |
| `node --experimental-strip-types scripts/sdkwork-apps-contract.test.ts` | passed | app-store package contract still enforces `@sdkwork/agentstudio-pc-core` root consumption and installer ownership |
