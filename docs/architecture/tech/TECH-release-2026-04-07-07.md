...
- The first clean-clone failure was `scripts/openclaw-release-contract.test.mjs`, which assumed `packages/sdkwork-clawstudio-desktop/src-tauri/resources/openclaw/manifest.json` already existed in a clean checkout even though that runtime manifest is generated during preparation.
