...
- Full `pnpm.cmd lint` moved beyond TypeScript and exposed a package-surface parity chain:
  - browser-root shared app-SDK wrappers in `@sdkwork/clawstudio-core` had drifted into lazy imports and Node-root re-exports
  ...
  - `sdkwork-clawstudio-devices` was missing the `@sdkwork/clawstudio-types` dependency declaration required by its package contract
- The loop repaired those boundary and contract drifts without changing product behavior:
  ...
  - removed browser-only wrappers from `packages/sdkwork-clawstudio-core/src/services/node/index.ts`
  ...
  - added `@sdkwork/clawstudio-types` to `packages/sdkwork-clawstudio-devices/package.json`
- Fresh verification:
...
- Current frontier:
  - `packages/sdkwork-clawstudio-instances/package.json` still lacks the `@monaco-editor/react` dependency expected by the IDE-style files workspace contract
...
- Full workspace lint still does not close because the next parity blocker remains in `sdkwork-clawstudio-instances`.
