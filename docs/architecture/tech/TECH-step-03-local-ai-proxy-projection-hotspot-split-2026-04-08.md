> Migrated from `docs/review/step-03-local-ai-proxy-projection-hotspot-split-2026-04-08.md` on 2026-06-24.
...
- Root cause:
  - `packages/sdkwork-agentstudio-pc-desktop/src-tauri/src/framework/services/local_ai_proxy.rs` still mixed loopback proxy lifecycle, protocol translation, observability, and managed OpenClaw provider projection into one large runtime hotspot.
  ...
  - after the split work, fresh `pnpm.cmd check:desktop` exposed a second real issue: `scripts/openclaw-release-contract.test.mjs` still treated `packages/sdkwork-agentstudio-pc-tasks/src/services/taskService.test.ts` as an OpenClaw version-fixture owner even though that file had already been reduced to a thin wrapper contract test.
- Implemented the narrow repair:
  - added `packages/sdkwork-agentstudio-pc-desktop/src-tauri/src/framework/services/local_ai_proxy/projection.rs` and moved the managed-provider projection / `openclaw.json` helper stack into that module
  ...
  - removed `packages/sdkwork-agentstudio-pc-tasks/src/services/taskService.test.ts` from `scripts/openclaw-release-contract.test.mjs` so the release-metadata contract again points only at files that still own shared bundled-version fixtures
