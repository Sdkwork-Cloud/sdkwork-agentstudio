...
- `packages/sdkwork-clawstudio-desktop/src-tauri/src/framework/openclaw_release.rs` 又引入了编译期版本投影。
- UI 显示版本、authority 状态、active 状态、manifest 版本之间缺少统一优先级规则，容易产生版本漂移。

### 2.2 目录模型不是多内核标准化模型

- `packages/sdkwork-clawstudio-desktop/src-tauri/src/framework/paths.rs` 中存在大量 `openclaw_*` 顶层路径字段。
