...
- The loop repaired one remaining local proxy streaming hotspot:
  - `packages/sdkwork-clawstudio-desktop/src-tauri/src/framework/services/local_ai_proxy.rs` still owned shared stream detection, passthrough response construction, translated SSE/JSONL builders, and provider stream-frame handling even though those helpers served multiple request paths
