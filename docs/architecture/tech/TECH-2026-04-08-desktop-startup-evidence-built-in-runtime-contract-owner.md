> Migrated from `docs/架构/126-2026-04-08-desktop-startup-evidence-built-in-runtime-contract-owner.md` on 2026-06-24.
> Owner: SDKWork maintainers

# 126-2026-04-08 Desktop Startup Evidence Built-In Runtime Contract Owner

## Decision

If persisted desktop startup evidence already contains shell-relevant built-in instance runtime contract fields, the desktop kernel/runtime publication chain must preserve them instead of truncating them before shell consumption.
...
- `builtInInstanceRuntimeKind` is the startup-time runtime identity. Without it, the shell cannot show that the persisted startup snapshot validated an `openclaw` runtime specifically.
- `builtInInstanceDeploymentMode` is the startup-time deployment ownership truth. Without it, the shell cannot show that the persisted startup snapshot validated the built-in instance under `local-managed` semantics.
- Both facts are already used by Step 03 readiness and management rules in the hosted desktop chain, so dropping them in the kernel publication layer creates a false boundary where raw truth exists but the shared runtime contract silently hides it.
...
- `Kernel Center` may present the published built-in instance runtime contract fields, but it may not:
  - become a second parser for the raw evidence document
  - infer a synthetic runtime kind
  - infer deployment mode from current live lifecycle state

## Impact

- `CP03-4` now has an explicit rule for shell-facing startup-evidence built-in runtime contract truth.
...
- The desktop shell gains better startup contract visibility while preserving the repository's host-to-feature dependency direction.
