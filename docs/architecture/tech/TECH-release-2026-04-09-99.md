...
- Implemented the Step 07 extraction:
  - added `packages/sdkwork-clawstudio-instances/src/services/openClawAgentSkillMutationSupport.ts`
  - rewired `packages/sdkwork-clawstudio-instances/src/pages/InstanceDetail.tsx` so the page now builds one injected agent skill runner from the shared helper and removes the inline pending/toast/reload orchestration from the three skill handlers
  - added `packages/sdkwork-clawstudio-instances/src/services/openClawAgentSkillMutationSupport.test.ts`
