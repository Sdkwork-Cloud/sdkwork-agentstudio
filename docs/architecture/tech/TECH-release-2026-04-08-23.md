...
- The loop repaired one remaining local proxy request-translation hotspot:
  - `packages/sdkwork-clawstudio-desktop/src-tauri/src/framework/services/local_ai_proxy.rs` still owned shared OpenAI request text extraction, conversation normalization, max-token shaping, and Anthropic/Gemini/Ollama request builders even though those helpers served multiple translated request paths
