> Migrated from `docs/review/step-03-runtime-barrel-root-export-and-wrapper-contract-alignment-2026-04-08.md` on 2026-06-24.
...
- `sdkwork-agentstudio-pc-instances/src/services/index.ts` now exports runtime surfaces only, and `agentWorkbenchService.test.ts` now returns a typed tools catalog agent id instead of collapsing to `unknown`.
...
- Package roots now explicitly publish the shell-facing surfaces that the shell already consumes through package roots:
  - `@sdkwork/agentstudio-pc-auth`: `AuthPage`, `AuthOAuthCallbackPage`
  - `@sdkwork/agentstudio-pc-center`: `ClawCenter`, `ClawDetail`, `ClawUpload`
  - `@sdkwork/agentstudio-pc-points`: `PointsHeaderEntry`
  - `@sdkwork/agentstudio-pc-chat`: `OpenClawGatewayConnections`, `useChatStore`
- `clawMallService.test.ts` now builds full `ClawMallProduct` fixtures instead of relying on `as any`.
- `taskService.test.ts` now validates the current thin-wrapper contract of `@sdkwork/agentstudio-pc-tasks` instead of targeting a retired injected `createTaskService(...)` factory that no longer exists in the package surface.
...
- The highest-risk runtime change is package-surface alignment:
  - `sdkwork-agentstudio-pc-instances/src/services/index.ts` no longer re-exports `.test` or `*Core` modules.
  - auth/center/points/chat package roots now explicitly publish shell-consumed runtime surfaces.
- `sdkwork-agentstudio-pc-tasks` now documents and tests its thin-wrapper contract instead of a retired injected factory shape.
