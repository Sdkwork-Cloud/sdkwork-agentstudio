...
- The loop repaired one remaining local proxy control-plane hotspot:
  - `packages/sdkwork-agentstudio-pc-desktop/src-tauri/src/framework/services/local_ai_proxy.rs` still owned route-probe orchestration, capability classification, protocol-specific probe HTTP construction, and probe response success parsing even though those concerns are distinct from request-serving, observability, and projection logic
