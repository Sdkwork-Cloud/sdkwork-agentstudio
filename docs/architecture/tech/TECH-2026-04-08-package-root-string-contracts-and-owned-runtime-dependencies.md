> Migrated from `docs/架构/101-2026-04-08-package-root-string-contracts-and-owned-runtime-dependencies.md` on 2026-06-24.
...
- `packages/sdkwork-agentstudio-pc-settings/src/index.ts` explicitly publishes `KernelCenter.ts` and `ProviderConfigCenter.ts` through the exact contract-frozen paths.
- `packages/sdkwork-agentstudio-pc-instances/src/index.ts` explicitly publishes `./pages/Nodes.tsx` for the shell route contract.
- `packages/sdkwork-agentstudio-pc-tasks/src/index.ts` explicitly publishes `./components/GlobalTaskManager`, `./services/taskService`, and `./store/useTaskStore`.
- `packages/sdkwork-agentstudio-pc-instances/package.json` declares `@monaco-editor/react` and `monaco-editor` because the instances workbench lazily loads Monaco at runtime.
...
- `sdkwork-agentstudio-pc-instances` again owns the Monaco editor dependency required by its IDE-style workbench.
