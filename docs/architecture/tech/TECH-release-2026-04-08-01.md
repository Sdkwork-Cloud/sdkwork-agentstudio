...
- 在不放宽 `@sdkwork/agentstudio-pc-core` Node-safe root 入口边界的前提下，修复 guided install 在 Node 合约测试与离线目录场景下的可用性。
...
- `cargo test --manifest-path packages/sdkwork-agentstudio-pc-desktop/src-tauri/Cargo.toml --target-dir target/step03-proof desktop_kernel_info_exposes_extended_runtime_directories`
