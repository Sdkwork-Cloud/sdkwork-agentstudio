...
- The loop repaired one remaining local proxy control-plane hotspot:
  - `packages/sdkwork-agentstudio-pc-desktop/src-tauri/src/framework/services/local_ai_proxy.rs` still owned config-file loading, public-host normalization, and loopback-safe host resolution even though those concerns are distinct from request-serving, observability, and projection logic
