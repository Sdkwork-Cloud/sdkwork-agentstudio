...
- Implemented the Step 07 extraction:
  - extended `packages/sdkwork-clawstudio-instances/src/services/openClawProviderCatalogMutationSupport.ts` with `createOpenClawProviderCatalogMutationRunner(...)`
  - rewired `packages/sdkwork-clawstudio-instances/src/pages/InstanceDetail.tsx` so the page now builds one injected provider-catalog runner from the shared helper and removes the inline completion/switch cluster
  - extended `packages/sdkwork-clawstudio-instances/src/services/openClawProviderCatalogMutationSupport.test.ts`
