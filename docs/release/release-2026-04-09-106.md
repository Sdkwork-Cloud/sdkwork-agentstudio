...
- Implemented the Step 07 extraction:
  - added `packages/sdkwork-clawstudio-instances/src/services/openClawProviderPresentation.ts`
  - updated `packages/sdkwork-clawstudio-instances/src/services/index.ts`
  - rewired `packages/sdkwork-clawstudio-instances/src/pages/InstanceDetail.tsx` so the page now gets provider create/edit dialog draft state from the shared helper instead of using raw provider draft factories inline
  - added `packages/sdkwork-clawstudio-instances/src/services/openClawProviderPresentation.test.ts`
