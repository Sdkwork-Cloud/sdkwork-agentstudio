...
当前工作区在按职责拆分为业务包的同时，继续保持与 `upgrade/agent-studio-v5` 基本一致的产品界面，并通过共享 Shell 同时服务 Web 与桌面端。
...
迁移目标是在功能和视觉上尽量与 `upgrade/agent-studio-v5` 保持一致。分包本身是为了提升维护性，而不是为了重做产品设计。
