...
| `pnpm check:parity` | Run focused parity checks against critical `upgrade/agent-studio-v5` behavior |
| `pnpm sdk:sync-features` | Sync feature package wiring helpers maintained by repository scripts |

## Desktop Commands

| Command | Purpose |
| --- | --- |
| `pnpm dev:desktop` | Run the desktop shell and launch Tauri |
| `pnpm build:desktop` | Build desktop installers and bundles |
...
| `pnpm release:package:desktop` | Collect already-built desktop installers and checksum files into `artifacts/release`; run `pnpm release:desktop` or `pnpm build:desktop` first |
...
| Command | Purpose |
| --- | --- |
| `pnpm dev:server` | Run the native Rust server host in development mode |
| `pnpm build:server` | Build the native Rust server binary in release mode; append `-- --target <triple>` for an explicit release target. Windows hosts automatically route Linux targets through WSL when an installed distro is available |
...
Use pnpm's filters to target one package:
...
- `push` tags matching `release-*` trigger a full Agent Studio release
