# 执行计划：claw-studio → sdkwork-clawstudio 对齐 sdkwork-specs
...
前置条件：停止所有 dev server / Tauri / cargo 进程，关闭 IDE 对 `claw-studio` 的文件监视（当前因 node/cargo 进程占用导致 `Move-Item` 访问被拒绝）。

```powershell
# 在干净的 PowerShell（无 IDE/dev server 占用）中执行
Move-Item -Path "E:\sdkwork-space\claw-studio" -Destination "E:\sdkwork-space\sdkwork-clawstudio"
```

校验：
- `sdkwork-clawstudio/AGENTS.md` 中 `../sdkwork-specs` 相对路径仍可解析 ✅（兄弟目录不变）
- `sdkwork-clawstudio/pnpm-workspace.yaml` 中 `../sdkwork-iam`、`../sdkwork-drive` 等兄弟相对路径仍有效 ✅
...
目标：`sdkwork-clawstudio-server` 经框架装配 HTTP 运行时，替换裸 axum。权威：`sdkwork-web-framework/specs/WEB_FRAMEWORK_STANDARD.md`。

2.1 依赖（`sdkwork-clawstudio-server/src-host/Cargo.toml`）：
```toml
...
目标：`sdkwork-clawstudio-host-core` 经 `sdkwork-database` 管理生命周期，达 L2（Contract Governed）。权威：`sdkwork-database/specs/DATABASE_FRAMEWORK_STANDARD.md`。【Human Review Gate：schema 变更】

3.1 资产字典（`sdkwork-clawstudio-host-core/database/`）：
...
3.3 表命名对齐 DATABASE_SPEC：`host_catalog_documents` → `clawstudio_host_catalog_documents`（域前缀），列逻辑类型替换裸 SQL 类型。

校验：`sdkwork-database-cli validate`、`migrate`、`seed`、`drift`；`cargo test -p sdkwork-clawstudio-host-core`。
...
4.2 TS：`sdkwork-clawstudio-pc-core` 等包 `dependencies` 加 `@sdkwork/utils`（`../sdkwork-utils/packages/sdkwork-utils-typescript`）；`chatUploadService.ts` 的 `defaultCreateId`/`sanitizePathSegment`/`sanitizeFileName`/`numberFromString` 替换为 `@sdkwork/utils` 对应方法；`infrastructure/http/httpClient.ts` 保留（基础设施层），但错误归一化用 utils。
