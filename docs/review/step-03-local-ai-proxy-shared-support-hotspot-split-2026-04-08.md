...
- Root cause:
  - `packages/sdkwork-clawstudio-desktop/src-tauri/src/framework/services/local_ai_proxy.rs` still mixed generic runtime lifecycle and router assembly with one remaining shared support hotspot:
    - `proxy_error(...)`
    - `duration_to_ms(...)`
    - `trim_optional_text(...)`
    - `current_time_ms(...)`
...
- Implemented the narrow repair:
  - added `packages/sdkwork-clawstudio-desktop/src-tauri/src/framework/services/local_ai_proxy/support.rs` as the dedicated owner for:
