...
- Root cause:
  - `packages/sdkwork-agentstudio-pc-desktop/src-tauri/src/framework/services/local_ai_proxy.rs` still mixed generic runtime orchestration with the whole OpenAI-compatible serving stack even after the earlier config, projection, probe, upstream, streaming, request-translation, response-translation, observability, Gemini native, Anthropic native, and health/status splits.
...
- Implemented the narrow repair:
  - kept `packages/sdkwork-agentstudio-pc-desktop/src-tauri/src/framework/services/local_ai_proxy/openai_compatible.rs` as the dedicated owner for:
    - `/v1/models`
    - `/v1/chat/completions`
    - `/v1/responses`
    - `/v1/embeddings`
