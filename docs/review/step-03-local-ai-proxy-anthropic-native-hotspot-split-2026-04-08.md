...
- Root cause:
  - `packages/sdkwork-agentstudio-pc-desktop/src-tauri/src/framework/services/local_ai_proxy.rs` still mixed generic runtime orchestration with the Anthropic native `/v1/messages` request path even after the earlier config, probe, upstream, streaming, request-translation, response-translation, observability, and Gemini native splits.
