...
- Step 03 continued from the `sdkwork-agentstudio-pc-instances` IDE-workbench dependency frontier into the next shell and tasks package-root contract layer.
...
- Implemented the narrow repairs:
  - restored `@monaco-editor/react` and `monaco-editor` to `packages/sdkwork-agentstudio-pc-instances/package.json` so the IDE-style workbench owns its Monaco runtime explicitly again
  - changed `packages/sdkwork-agentstudio-pc-settings/src/index.ts` to publish `KernelCenter.ts` and `ProviderConfigCenter.ts` through the exact contract-frozen paths
  - changed `packages/sdkwork-agentstudio-pc-instances/src/index.ts` to publish `./pages/Nodes.tsx` explicitly for the shell route contract
  - changed `packages/sdkwork-agentstudio-pc-tasks/src/index.ts` to publish `GlobalTaskManager`, `taskService`, and `useTaskStore` from the package root instead of relying on indirect reachability
