...
- Removed the now-redundant page-local imports of:
  ...
  - `InstanceDetailAgentsSection`
  - `InstanceDetailManagedLlmProvidersSection`
- Kept `packages/sdkwork-agentstudio-pc-instances/src/components/instanceDetailSectionModels.ts` as the single pure composition owner for:
...
- The focused helper coverage already added in `packages/sdkwork-agentstudio-pc-instances/src/components/instanceDetailSectionModels.test.tsx` remains aligned with the new builder boundary.
...
- `packages/sdkwork-agentstudio-pc-instances/src/components/instanceDetailSectionModels.ts`: `435`
