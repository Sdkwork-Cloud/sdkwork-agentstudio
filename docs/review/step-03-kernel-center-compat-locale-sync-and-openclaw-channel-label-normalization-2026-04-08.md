...
- Synced the compatibility locale bundles `packages/sdkwork-clawstudio-i18n/src/locales/en.json` and `packages/sdkwork-clawstudio-i18n/src/locales/zh.json` with the split settings locale resources so `Kernel Center` startup-evidence copy is visible to aggregate-bundle consumers again.
...
- Root cause 1:
  - `packages/sdkwork-clawstudio-i18n/src/locales/en/settings.json` and `packages/sdkwork-clawstudio-i18n/src/locales/zh/settings.json` already contained the new startup-evidence section and field labels introduced by the recent `Kernel Center` loops.
  - compatibility aggregate bundles `packages/sdkwork-clawstudio-i18n/src/locales/en.json` and `packages/sdkwork-clawstudio-i18n/src/locales/zh.json` were still missing those keys.
...
- Root cause 2:
  - once the locale aggregates were resynced, `packages/sdkwork-clawstudio-i18n/src/index.test.ts` exposed the next real blocker.
  - `packages/sdkwork-clawstudio-core/src/services/openClawConfigService.ts` still embedded Chinese channel names (`SDKWORK公众号`, `微信公众号`) in source constants.
...
- Implemented the narrow repair:
  - reran `pnpm.cmd --filter @sdkwork/clawstudio-i18n sync:locales` to regenerate compatibility aggregate bundles from the split locale files.
