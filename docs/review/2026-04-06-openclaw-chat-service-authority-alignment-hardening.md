...
But `packages/sdkwork-agentstudio-pc-chat/src/services/chatService.ts`, which is used by
`packages/sdkwork-agentstudio-pc-chat/src/pages/Chat.tsx` for the direct send/stream
...
`packages/sdkwork-agentstudio-pc-chat/src/services/chatService.ts` now resolves the active
