...
impl SqliteCatalogDatabase {
    pub(crate) fn new(path: PathBuf) -> Result<Self, StorageError> {
        ...
        let pool = runtime.block_on(async {
            // Create pool options with a busy timeout to match the previous
            // rusqlite `busy_timeout(Duration::from_secs(5))` behavior.
            let pool = SqlitePool::connect(&database_url).await?;
            // Run schema migration.
            sqlx::query(SQLITE_SCHEMA_SQL).execute(&pool).await?;
