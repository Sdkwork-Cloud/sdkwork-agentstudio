...
await runTest('api settings runtime summary treats ready local proxy lifecycle as a first-class localized state', async () => {
  const apiSettingsSource = read('./ApiSettings.tsx');
  const enApiLogs = JSON.parse(read('../../sdkwork-clawstudio-i18n/src/locales/en/apiLogs.json')) as Record<string, any>;
  const zhApiLogs = JSON.parse(read('../../sdkwork-clawstudio-i18n/src/locales/zh/apiLogs.json')) as Record<string, any>;
