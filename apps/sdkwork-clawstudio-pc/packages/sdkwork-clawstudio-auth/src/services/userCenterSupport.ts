...
export const USER_CENTER_SOURCE_PACKAGE_NAME = '@sdkwork/clawstudio-auth';
export const USER_CENTER_VALIDATION_SOURCE_PACKAGE_NAME = '@sdkwork/clawstudio-auth';
...
function createDefaultProviderKey(kind: UserCenterProviderKind) {
  switch (kind) {
    case 'external-user-center':
      return 'claw-studio-external-user-center';
    case 'builtin-local':
      return 'claw-studio-local';
    default:
      return 'claw-studio-app-api';
  }
}
...
export function createUserCenterBridgeConfig(
  ...
  const namespace = normalizeOptionalString(input.namespace) ?? 'claw-studio';
...
export function createUserCenterPluginDefinition(
  ...
  const builtinLocal = createDeploymentProfile(
    'builtin-local',
    slugifyIdentifier('claw-studio-local', 'claw-studio-local'),
  );
  ...
  const externalUserCenter = createDeploymentProfile(
    ...
    bridgeConfig.provider.kind === 'external-user-center'
      ? bridgeConfig.provider.providerKey
      : 'claw-studio-external-user-center',
  );
