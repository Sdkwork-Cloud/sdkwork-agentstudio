...
`@sdkwork/clawstudio-auth` owns auth page composition only:
...
It does not own raw HTTP, token persistence, or backend contract handling. Those stay in `@sdkwork/clawstudio-core` and the generated `retired generic app SDK package` path.
...
```tsx
import { AuthPage, AuthOAuthCallbackPage } from '@sdkwork/clawstudio-auth';
...
- all remote auth actions flow through `@sdkwork/clawstudio-core`
...
- keep remote business calls in `@sdkwork/clawstudio-core`
