...
test('resolveWorkspacePackageEntry maps @sdkwork workspace packages into the current workspace', () => {
  assert.equal(
    resolveWorkspacePackageEntry('@sdkwork/clawstudio-infrastructure', packagesRoot),
    path.resolve(packagesRoot, 'sdkwork-clawstudio-infrastructure/src/index.ts'),
  );
  assert.equal(
    resolveWorkspacePackageEntry('@sdkwork/clawstudio-i18n', packagesRoot),
    path.resolve(packagesRoot, 'sdkwork-clawstudio-i18n/src/index.ts'),
  );
  assert.equal(
...
test('resolveWorkspacePackageAliases creates direct aliases for local @sdkwork/clawstudio-* packages', () => {
  const aliases = resolveWorkspacePackageAliases(packagesRoot);
  const infrastructureAlias = aliases.find((entry) => entry.find === '@sdkwork/clawstudio-infrastructure');
  const i18nAlias = aliases.find((entry) => entry.find === '@sdkwork/clawstudio-i18n');
...
test('remapWorktreeWorkspaceImport remaps absolute worktree package paths into the current workspace', () => {
  ...
  assert.equal(
    ...
    path.resolve(packagesRoot, 'sdkwork-clawstudio-infrastructure/src/platform/webStudio.ts'),
  );
});
...
test('remapWorktreeWorkspaceImport remaps relative imports when the importer comes from a worktree package', () => {
  const worktreeImporter = path.resolve(
    worktreeRoot,
    'packages/sdkwork-clawstudio-infrastructure/src/services/fileDialogService.ts',
  );

  assert.equal(
    remapWorktreeWorkspaceImport('../platform/index.ts', worktreeImporter, packagesRoot),
    path.resolve(packagesRoot, 'sdkwork-clawstudio-infrastructure/src/platform/index.ts'),
  );
});
...
test('remapWorktreeWorkspaceImport resolves extensionless relative imports to existing source files', () => {
  ...
  assert.equal(
    ...
    path.resolve(packagesRoot, 'sdkwork-clawstudio-web/src/App.tsx'),
  );
  assert.equal(
    remapWorktreeWorkspaceImport('./application/app/AppRoot', path.resolve(worktreeRoot, 'packages/sdkwork-clawstudio-shell/src/index.ts'), packagesRoot),
    path.resolve(packagesRoot, 'sdkwork-clawstudio-shell/src/application/app/AppRoot.tsx'),
  );
});
...
test('remapWorktreeWorkspaceImport preserves Vite /@fs/ prefixes and query suffixes', () => {
  ...
  assert.equal(
    ...
    `${path.resolve(packagesRoot, 'sdkwork-clawstudio-i18n/src/index.ts')}?v=worktree`,
  );
});
...
test('shouldAttemptWorkspaceResolverRemap fast-rejects imports that cannot target the workspace remap paths', () => {
  ...
  assert.equal(shouldAttemptWorkspaceResolverRemap('@sdkwork/clawstudio-infrastructure', undefined), false);
  assert.equal(
    shouldAttemptWorkspaceResolverRemap(
      path.resolve(worktreeRoot, 'packages/sdkwork-clawstudio-infrastructure/src/platform/webStudio.ts'),
      undefined,
      packagesRoot,
    ),
    true,
