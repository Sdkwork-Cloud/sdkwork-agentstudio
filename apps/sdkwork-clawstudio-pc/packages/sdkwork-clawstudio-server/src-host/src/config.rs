...
pub const CLAW_SERVER_DEFAULT_DATA_DIR: &str = ".clawstudio-server";
pub const CLAW_SERVER_DEFAULT_WEB_DIST_DIR: &str = "../sdkwork-clawstudio-web/dist";
pub const CLAW_SERVER_DEFAULT_CONFIG_FILE_NAME: &str = "clawstudio-server.config.json";
...
fn resolve_packaged_server_bundle_paths(
    ...
    let executable_name = executable_path
        ...
        .trim()
        .to_ascii_lowercase();
    if executable_name != "clawstudio-server" && executable_name != "clawstudio-server.exe" {
        return None;
    }
...
mod tests {
    ...
    fn runtime_config_uses_packaged_bundle_defaults_when_running_bundled_binary() {
        ...
        let executable_path = bundle_root.join("bin").join("clawstudio-server");
        let web_dist_dir = bundle_root.join("web").join("dist");
        ...
        assert_eq!(resolved.data_dir, bundle_root.join(".clawstudio-server"));
    ...
    fn runtime_config_ignores_packaged_shape_without_bundled_web_dist() {
        ...
        assert_eq!(resolved.data_dir, PathBuf::from(".clawstudio-server"));
        assert_eq!(
            resolved.web_dist_dir,
    ...
    fn unique_missing_config_path(label: &str) -> PathBuf {
        ...
        env::temp_dir().join(format!(
            "sdkwork-clawstudio-server-{label}-{}-{unique_suffix}.json",
            std::process::id()
        ))
    }
    ...
    fn unique_bundle_root(label: &str) -> PathBuf {
        let unique_suffix = SystemTime::now()
            ...
            .as_nanos();
        env::temp_dir().join(format!(
            "sdkwork-clawstudio-server-bundle-{label}-{}-{unique_suffix}",
            std::process::id()
        ))
    }
}
