...
await runTest('sdkwork-clawstudio-core node root entry loads under Node without browser-only runtime wiring', async () => {
  const moduleUrl = pathToFileURL(path.join(process.cwd(), 'packages/sdkwork-clawstudio-core/src/node.ts')).href;
