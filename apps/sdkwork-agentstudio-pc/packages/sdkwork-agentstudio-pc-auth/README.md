# @sdkwork/agentstudio-pc-auth

Agent Studio auth center for web and desktop hosts.
...
`@sdkwork/agentstudio-pc-auth` owns auth page composition only:
...
It does not own raw HTTP, token persistence, or backend contract handling. Those stay in `@sdkwork/agentstudio-pc-core` and the generated `retired generic app SDK package` path.
...
} from '@sdkwork/agentstudio-pc-auth';
```
...
```tsx
import { AuthPage, AuthOAuthCallbackPage } from '@sdkwork/agentstudio-pc-auth';
...
- all remote auth actions flow through `@sdkwork/agentstudio-pc-core`
...
- keep remote business calls in `@sdkwork/agentstudio-pc-core`
