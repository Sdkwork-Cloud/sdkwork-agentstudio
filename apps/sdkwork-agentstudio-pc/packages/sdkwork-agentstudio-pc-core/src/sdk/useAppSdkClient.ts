...
import {
  ...
  AGENT_STUDIO_USER_CENTER_STORAGE_PLAN,
} from './userCenterContract.ts';
import { createUserCenterTokenStore } from './userCenterStorage.ts';
import type { agentstudioRemoteAppClient } from './appSdkPort.ts';
...
export type agentstudioAppClient = agentstudioRemoteAppClient;
...
export const APP_SDK_SESSION_STORAGE_KEY = 'agent-studio-auth-session';
export const APP_SDK_USER_CENTER_NAMESPACE = AGENT_STUDIO_USER_CENTER_NAMESPACE;
export const APP_SDK_USER_CENTER_STORAGE_PLAN = AGENT_STUDIO_USER_CENTER_STORAGE_PLAN;

let runtimeConfigured = false;
let appSdkRuntime: AppSdkRuntime | null = null;

const AGENT_STUDIO_APP_ID = '10001';
...
export function getagentstudioAppClientWithSession(
  overrides: Partial<SdkworkAppConfig> = {},
): agentstudioAppClient {
  ensureConfigured();
  return getRuntimeAppClientWithSession(overrides) as agentstudioAppClient;
}
