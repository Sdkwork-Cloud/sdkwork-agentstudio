...
test('resolveWorkspacePackageEntry maps @sdkwork workspace packages into the current workspace', () => {
  assert.equal(
    resolveWorkspacePackageEntry('@sdkwork/agentstudio-pc-infrastructure', packagesRoot),
    path.resolve(packagesRoot, 'sdkwork-agentstudio-pc-infrastructure/src/index.ts'),
  );
  assert.equal(
    resolveWorkspacePackageEntry('@sdkwork/agentstudio-pc-i18n', packagesRoot),
    path.resolve(packagesRoot, 'sdkwork-agentstudio-pc-i18n/src/index.ts'),
  );
  assert.equal(
...
test('resolveWorkspacePackageAliases creates direct aliases for local @sdkwork/agentstudio-pc-* packages', () => {
  const aliases = resolveWorkspacePackageAliases(packagesRoot);
  const infrastructureAlias = aliases.find((entry) => entry.find === '@sdkwork/agentstudio-pc-infrastructure');
  const i18nAlias = aliases.find((entry) => entry.find === '@sdkwork/agentstudio-pc-i18n');
...
test('remapWorktreeWorkspaceImport remaps absolute worktree package paths into the current workspace', () => {
  ...
  assert.equal(
    ...
    path.resolve(packagesRoot, 'sdkwork-agentstudio-pc-infrastructure/src/platform/webStudio.ts'),
  );
});
...
test('remapWorktreeWorkspaceImport remaps relative imports when the importer comes from a worktree package', () => {
  const worktreeImporter = path.resolve(
    worktreeRoot,
    'packages/sdkwork-agentstudio-pc-infrastructure/src/services/fileDialogService.ts',
  );

  assert.equal(
    remapWorktreeWorkspaceImport('../platform/index.ts', worktreeImporter, packagesRoot),
    path.resolve(packagesRoot, 'sdkwork-agentstudio-pc-infrastructure/src/platform/index.ts'),
  );
});
...
test('remapWorktreeWorkspaceImport resolves extensionless relative imports to existing source files', () => {
  ...
  assert.equal(
    ...
    path.resolve(packagesRoot, 'sdkwork-agentstudio-pc-web/src/App.tsx'),
  );
  assert.equal(
    remapWorktreeWorkspaceImport('./application/app/AppRoot', path.resolve(worktreeRoot, 'packages/sdkwork-agentstudio-pc-shell/src/index.ts'), packagesRoot),
    path.resolve(packagesRoot, 'sdkwork-agentstudio-pc-shell/src/application/app/AppRoot.tsx'),
  );
});
...
test('remapWorktreeWorkspaceImport preserves Vite /@fs/ prefixes and query suffixes', () => {
  ...
  assert.equal(
    ...
    `${path.resolve(packagesRoot, 'sdkwork-agentstudio-pc-i18n/src/index.ts')}?v=worktree`,
  );
});
...
test('shouldAttemptWorkspaceResolverRemap fast-rejects imports that cannot target the workspace remap paths', () => {
  ...
  assert.equal(shouldAttemptWorkspaceResolverRemap('@sdkwork/agentstudio-pc-infrastructure', undefined), false);
  assert.equal(
    shouldAttemptWorkspaceResolverRemap(
      path.resolve(worktreeRoot, 'packages/sdkwork-agentstudio-pc-infrastructure/src/platform/webStudio.ts'),
      undefined,
      packagesRoot,
    ),
    true,
