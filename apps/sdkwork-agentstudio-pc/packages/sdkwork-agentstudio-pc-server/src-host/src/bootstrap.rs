...
use sdkwork_agentstudio_host_core::internal::node_sessions::NodeSessionRegistry;
use sdkwork_agentstudio_host_core::openclaw_control_plane::{
    OpenClawControlPlane, OpenClawGatewayInvokeRequest,
};
use sdkwork_agentstudio_host_core::rollout::control_plane::RolloutControlPlane;
use sdkwork_agentstudio_host_studio::{build_default_studio_public_api_provider, StudioPublicApiProvider};
use serde::Serialize;
...
fn try_build_server_state() -> Result<ServerState, String> {
    let rollout_data_dir = env::var("CLAW_SERVER_DATA_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from(".agentstudio-server"));
...
fn try_build_server_state_with_overrides(
    ...
    let executable_path = overrides.executable_path.clone().unwrap_or_else(|| {
        std::env::current_exe().unwrap_or_else(|_| PathBuf::from("agentstudio-server"))
    });
    let runtime_contract = ServerRuntimeContract {
        platform: current_service_platform(),
        executable_path,
...
pub fn resolve_server_web_dist_dir(value: Option<String>) -> PathBuf {
    value
        ...
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("../sdkwork-agentstudio-pc-web/dist"))
}
...
fn validate_public_bind_configuration(config: &ResolvedServerRuntimeConfig) -> Result<(), String> {
    ...
    Err(
        "refusing to start agentstudio-server on a non-loopback host without control-plane credentials; set CLAW_SERVER_MANAGE_USERNAME and CLAW_SERVER_MANAGE_PASSWORD (and optionally CLAW_SERVER_INTERNAL_USERNAME / CLAW_SERVER_INTERNAL_PASSWORD) or explicitly opt in with CLAW_SERVER_ALLOW_INSECURE_PUBLIC_BIND=true".to_string(),
    )
}
...
fn build_server_openclaw_control_plane(
    ...
    OpenClawControlPlane::inactive("agentstudio-server").with_host_endpoints(host_endpoints)
}
...
mod tests {
    ...
    use sdkwork_agentstudio_host_core::openclaw_control_plane::OpenClawControlPlane;
    ...
    fn resolve_server_web_dist_dir_prefers_explicit_value() {
        ...
        assert_eq!(
            ...
            PathBuf::from("../sdkwork-agentstudio-pc-web/dist"),
        );
    }
    ...
    fn runtime_contract_state_preserves_requested_and_active_server_ports_when_binding_falls_back()
    {
        ...
        let effective_config_path = rollout_data_dir.join("agentstudio-server.config.json");
        let executable_path = rollout_data_dir.join("agentstudio-server.exe");
        let state = build_server_state_from_runtime_contract(
            &crate::config::ResolvedServerRuntimeConfig {
                host: "127.0.0.1".to_string(),
                port: 28_901,
                data_dir: rollout_data_dir.clone(),
                web_dist_dir: PathBuf::from("../sdkwork-agentstudio-pc-web/dist"),
    ...
    fn create_test_rollout_data_dir(label: &str) -> PathBuf {
        ...
        let directory = env::temp_dir().join(format!(
            "sdkwork-agentstudio-standalone-gateway-bootstrap-{label}-{}-{unique_suffix}",
            std::process::id()
        ));
