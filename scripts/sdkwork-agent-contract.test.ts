...
runTest('sdkwork-agentstudio-pc-agent stays local and exposes a dedicated market surface', () => {
  const pkg = readJson<{ dependencies?: Record<string, string> }>(
    'packages/sdkwork-agentstudio-pc-agent/package.json',
  );
  const indexSource = read('packages/sdkwork-agentstudio-pc-agent/src/index.ts');
  const entrySource = read('packages/sdkwork-agentstudio-pc-agent/src/AgentMarket.tsx');
  const pageSource = read('packages/sdkwork-agentstudio-pc-agent/src/pages/AgentMarket.tsx');
  const servicesIndexSource = read('packages/sdkwork-agentstudio-pc-agent/src/services/index.ts');

  assert.ok(exists('packages/sdkwork-agentstudio-pc-agent/src/AgentMarket.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-agent/src/pages/AgentMarket.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-agent/src/services/agentCatalog.ts'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-agent/src/services/agentCatalog.test.ts'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-agent/src/services/agentInstallService.ts'));

  assert.ok(!pkg.dependencies?.['@sdkwork/agentstudio-pc-studio-agent']);
  assert.equal(pkg.dependencies?.['@sdkwork/agentstudio-pc-core'], 'workspace:*');
  assert.equal(pkg.dependencies?.['@sdkwork/agentstudio-pc-infrastructure'], 'workspace:*');
...
runTest('sdkwork-agentstudio-pc-agent delegates shared catalog and install logic to @sdkwork/agentstudio-pc-core while keeping a local market surface', () => {
  const catalogSource = read('packages/sdkwork-agentstudio-pc-agent/src/services/agentCatalog.ts');
  const installServiceSource = read('packages/sdkwork-agentstudio-pc-agent/src/services/agentInstallService.ts');
  const sharedCatalogSource = read('packages/sdkwork-agentstudio-pc-core/src/services/agentCatalog.ts');
  const sharedInstallServiceSource = read('packages/sdkwork-agentstudio-pc-core/src/services/agentInstallService.ts');
...
runTest('sdkwork-agentstudio-pc-agent parity checks use the shared Node TypeScript runner for OpenClaw install flows', () => {
