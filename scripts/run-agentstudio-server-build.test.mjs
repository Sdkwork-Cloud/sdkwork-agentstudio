...
test('server build helper creates a default release cargo plan', async () => {
  const modulePath = path.join(rootDir, 'scripts', 'run-agentstudio-server-build.mjs');
  ...
  assert.deepEqual(plan, {
    ...
    cwd: path.join(rootDir, 'packages', 'sdkwork-agentstudio-pc-server'),
    env: {},
    runner: 'run-cargo',
  });
});
...
test('server build helper routes Windows-hosted Linux targets through WSL when a distro is available', async () => {
  ...
  assert.equal(plan.cwd, path.join(rootDir, 'packages', 'sdkwork-agentstudio-pc-server'));
  ...
  assert.match(
    plan.args[5],
    new RegExp(`cd '${toExpectedWslPath(path.join(rootDir, 'packages', 'sdkwork-agentstudio-pc-server'))}'`),
  );
...
test('server build helper keeps WSL builds independent from the local Rust toolchain wrapper', async () => {
  ...
  assert.equal(spawnInvocation?.options.cwd, path.join(rootDir, 'packages', 'sdkwork-agentstudio-pc-server'));
...
test('server build helper removes stale legacy binaries after a successful native build', async () => {
  ...
  assert.deepEqual(
    ...
    [
      {
        targetPath: path.join(expectedReleaseDir, 'sdkwork-agentstudio-pc-server.exe'),
        options: { force: true },
      },
      {
        targetPath: path.join(expectedReleaseDir, 'sdkwork-agentstudio-pc-server'),
        options: { force: true },
      },
      {
        targetPath: path.join(expectedReleaseDir, 'sdkwork-agentstudio-pc-server.d'),
        options: { force: true },
      },
      {
        targetPath: path.join(expectedReleaseDir, 'sdkwork_agentstudio_server.pdb'),
        options: { force: true },
      },
    ],
  );
...
test('server build helper removes stale legacy binaries from both targeted and shared release directories for explicit target builds', async () => {
  ...
  assert.deepEqual(
    ...
    [
      {
        targetPath: path.join(expectedTargetReleaseDir, 'sdkwork-agentstudio-pc-server.exe'),
        options: { force: true },
      },
      {
        targetPath: path.join(expectedTargetReleaseDir, 'sdkwork-agentstudio-pc-server'),
        options: { force: true },
      },
      {
        targetPath: path.join(expectedTargetReleaseDir, 'sdkwork-agentstudio-pc-server.d'),
        options: { force: true },
      },
      {
        targetPath: path.join(expectedTargetReleaseDir, 'sdkwork_agentstudio_server.pdb'),
        options: { force: true },
      },
      {
        targetPath: path.join(expectedRootReleaseDir, 'sdkwork-agentstudio-pc-server.exe'),
        options: { force: true },
      },
      {
        targetPath: path.join(expectedRootReleaseDir, 'sdkwork-agentstudio-pc-server'),
        options: { force: true },
      },
      {
        targetPath: path.join(expectedRootReleaseDir, 'sdkwork-agentstudio-pc-server.d'),
        options: { force: true },
      },
      {
        targetPath: path.join(expectedRootReleaseDir, 'sdkwork_agentstudio_server.pdb'),
        options: { force: true },
      },
    ],
  );
