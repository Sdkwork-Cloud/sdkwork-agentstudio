...
runTest('desktop shell keeps title-bar window controls in the shared desktop bridge component', () => {
  const bridgeSource = readText('packages/sdkwork-agentstudio-pc-desktop/src/desktop/tauriBridge.ts');
  const controlsSource = readText('packages/sdkwork-agentstudio-pc-core/src/components/DesktopWindowControls.tsx');
  const headerSource = readText('packages/sdkwork-agentstudio-pc-shell/src/components/AppHeader.tsx');
  const startupSource = readText(
    'packages/sdkwork-agentstudio-pc-desktop/src/desktop/bootstrap/DesktopStartupScreen.tsx',
  );
...
runTest('desktop shell grants custom title-bar window permissions through a real capability file', () => {
  const capabilityRelativePath =
    'packages/sdkwork-agentstudio-pc-desktop/src-tauri/capabilities/default.json';
...
runTest('desktop capability stays aligned with the window APIs used by the desktop host', () => {
  const capability = readJson('packages/sdkwork-agentstudio-pc-desktop/src-tauri/capabilities/default.json');
  const bridgeSource = readText('packages/sdkwork-agentstudio-pc-desktop/src/desktop/tauriBridge.ts');
  const bootstrapSource = readText(
    'packages/sdkwork-agentstudio-pc-desktop/src/desktop/bootstrap/DesktopBootstrapApp.tsx',
  );
...
runTest('desktop shell keeps header interactions outside the drag region hitbox', () => {
  const headerSource = readText('packages/sdkwork-agentstudio-pc-shell/src/components/AppHeader.tsx');
  const switcherSource = readText('packages/sdkwork-agentstudio-pc-shell/src/components/InstanceSwitcher.tsx');
...
runTest('desktop runtime detection supports Tauri v2 window APIs without relying on withGlobalTauri', () => {
  const runtimeSource = readText('packages/sdkwork-agentstudio-pc-desktop/src/desktop/runtime.ts');
  const tauriConfig = readJson('packages/sdkwork-agentstudio-pc-desktop/src-tauri/tauri.conf.json');
...
runTest('desktop tray route bridge stays host-local and drives navigation through browser history', () => {
  const bridgeSource = readText(
    'packages/sdkwork-agentstudio-pc-desktop/src/desktop/bootstrap/DesktopTrayRouteBridge.tsx',
  );
...
runTest('desktop bridge exposes host language sync for tray localization', () => {
  const catalogSource = readText('packages/sdkwork-agentstudio-pc-desktop/src/desktop/catalog.ts');
  const bridgeSource = readText('packages/sdkwork-agentstudio-pc-desktop/src/desktop/tauriBridge.ts');
  const indexSource = readText('packages/sdkwork-agentstudio-pc-desktop/src/index.ts');
  ...
  const settingsSource = readText('packages/sdkwork-agentstudio-pc-settings/src/GeneralSettings.tsx');
  const appStoreSource = readText('packages/sdkwork-agentstudio-pc-core/src/stores/useAppStore.ts');
