...
const localeSourceDir = path.join(packagesDir, 'sdkwork-agentstudio-pc-i18n', 'src', 'locales');
...
const i18nIndexPath = path.join(packagesDir, 'sdkwork-agentstudio-pc-i18n', 'src', 'index.ts');
...
const webServerPathCandidates = [
  path.join(packagesDir, 'sdkwork-agentstudio-pc-web', 'server.ts'),
...
function checkLocaleOwnership() {
  ...
  if (localeFiles.length === 0) {
    fail('packages/sdkwork-agentstudio-pc-i18n/src/locales must exist and own locale resources.');
  }

  const duplicateLocaleFiles = listFiles(duplicateLocaleDir, (filePath) => filePath.endsWith('.json'));
  if (duplicateLocaleFiles.length > 0) {
    fail('packages/sdkwork-agentstudio-pc-infrastructure/src/i18n/locales must not exist; locale JSON must have a single owner.');
  }
}
...
function checkSupportedLocales() {
  ...
  if (appStoreSource.includes("'ja'")) {
    fail('packages/sdkwork-agentstudio-pc-core/src/stores/useAppStore.ts still exposes unsupported language "ja".');
  }

  const settingsSource = readFile(settingsPath);
  if (/value="ja"/.test(settingsSource)) {
    fail('packages/sdkwork-agentstudio-pc-settings/src/GeneralSettings.tsx still exposes a Japanese language option.');
  }
}
...
function checkRequestAwareRuntime() {
  ...
  if (webServerPath) {
    ...
    if (!/ensureI18n/.test(providersSource)) {
      fail(
        'packages/sdkwork-agentstudio-pc-shell/src/application/providers/AppProviders.tsx must bootstrap ensureI18n when no request-aware server runtime exists.',
      );
    }
  }
  ...
  if (!/cookie/i.test(i18nSource)) {
    fail('packages/sdkwork-agentstudio-pc-i18n/src/index.ts must configure cookie-based language detection.');
  }
  if (!/supportedLngs/i.test(i18nSource)) {
    fail('packages/sdkwork-agentstudio-pc-i18n/src/index.ts must declare supportedLngs.');
  }
}
...
function checkChineseOutsideLocales() {
  const sourceFiles = listFiles(packagesDir, (filePath) => {
    ...
    return (
      ...
      !normalized.includes('\\sdkwork-agentstudio-pc-i18n\\src\\locales\\') &&
      !normalized.includes('\\sdkwork-agentstudio-pc-infrastructure\\src\\i18n\\locales\\')
    );
  });
...
function checkCorruptedLocaleValues() {
  ...
  if (corruptedEntries.length > 0) {
    fail(
      `packages/sdkwork-agentstudio-pc-i18n/src/locales/zh.json contains corrupted placeholder values: ${corruptedEntries
        .slice(0, 10)
        .join(', ')}`,
    );
  }
  ...
  if (missingChineseAnchors.length > 0) {
    fail(
      `packages/sdkwork-agentstudio-pc-i18n/src/locales/zh.json contains mojibake or missing Chinese copy in: ${missingChineseAnchors.join(
        ', ',
      )}`,
    );
  }
