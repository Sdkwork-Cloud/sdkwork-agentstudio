...
runTest('sdkwork-agentstudio-pc-ui is implemented locally instead of re-exporting agent-studio-shared-ui', () => {
  const pkg = readJson<{ dependencies?: Record<string, string> }>('packages/sdkwork-agentstudio-pc-ui/package.json');
  const indexSource = read('packages/sdkwork-agentstudio-pc-ui/src/index.ts');
  const componentsIndexSource = read('packages/sdkwork-agentstudio-pc-ui/src/components/index.ts');
  const taskRowListSource = read('packages/sdkwork-agentstudio-pc-ui/src/components/TaskRowList.tsx');
  const taskCatalogSource = read('packages/sdkwork-agentstudio-pc-ui/src/components/TaskCatalog.tsx');
  const taskCatalogMetaSource = read('packages/sdkwork-agentstudio-pc-ui/src/components/taskCatalogMeta.ts');
  const taskExecutionHistoryDrawerSource = read(
    'packages/sdkwork-agentstudio-pc-ui/src/components/TaskExecutionHistoryDrawer.tsx',
  );
  const channelCatalogSource = read('packages/sdkwork-agentstudio-pc-ui/src/components/ChannelCatalog.tsx');
  const channelWorkspaceSource = read('packages/sdkwork-agentstudio-pc-ui/src/components/ChannelWorkspace.tsx');
  const channelBindingGuidesSource = read('packages/sdkwork-agentstudio-pc-ui/src/components/channelBindingGuides.ts');
  const channelRegionTabsSource = read('packages/sdkwork-agentstudio-pc-ui/src/components/ChannelRegionTabs.tsx');
  const channelCatalogMetaSource = read('packages/sdkwork-agentstudio-pc-ui/src/components/channelCatalogMeta.ts');
  const channelCatalogRegionContentSource = read(
    'packages/sdkwork-agentstudio-pc-ui/src/components/channelCatalogRegionContent.ts',
  );
  const overlaySurfaceSource = read('packages/sdkwork-agentstudio-pc-ui/src/components/OverlaySurface.tsx');

  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/Modal.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/Button.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/Input.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/Textarea.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/Label.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/Select.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/Dialog.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/Checkbox.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/Switch.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/Slider.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/RepositoryCard.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/TaskRowList.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/TaskCatalog.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/TaskExecutionHistoryDrawer.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/ChannelWorkspace.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/channelBindingGuides.ts'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/ChannelRegionTabs.tsx'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/channelCatalogRegionContent.ts'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/components/taskCatalogMeta.ts'));
  assert.ok(exists('packages/sdkwork-agentstudio-pc-ui/src/lib/utils.ts'));

  assert.ok(!pkg.dependencies?.['@sdkwork/agentstudio-pc-studio-shared-ui']);
  assert.doesNotMatch(indexSource, /@sdkwork\/agent-studio-shared-ui/);
...
runTest('shared dialog and overlay primitives expose stable surface slots for light theme depth', () => {
  const dialogSource = read('packages/sdkwork-agentstudio-pc-ui/src/components/Dialog.tsx');
  const overlaySurfaceSource = read('packages/sdkwork-agentstudio-pc-ui/src/components/OverlaySurface.tsx');
...
runTest('feature packages use shared shadcn-style form primitives instead of native controls', () => {
  ...
  const architectureAllowList = new Set([
    // @sdkwork/agentstudio-pc-core is not allowed to depend on @sdkwork/agentstudio-pc-ui by repo architecture rules.
    'packages/sdkwork-agentstudio-pc-core/src/components/CommandPalette.tsx',
  ]);
  const sourceFiles = walkFiles(
    packagesRoot,
    (filePath) =>
      filePath.endsWith('.tsx') &&
      !filePath.includes(`${path.sep}sdkwork-agentstudio-pc-ui${path.sep}src${path.sep}components${path.sep}`),
  );
  ...
  assert.deepEqual(
    violations,
    [],
    `Native form controls remain outside @sdkwork/agentstudio-pc-ui:\n${violations.join('\n')}`,
  );
});

runTest('shared form primitives keep interactive layers and focus states compatible with overlay drawers', () => {
  const overlaySurfaceSource = read('packages/sdkwork-agentstudio-pc-ui/src/components/OverlaySurface.tsx');
  const selectSource = read('packages/sdkwork-agentstudio-pc-ui/src/components/Select.tsx');
  const inputSource = read('packages/sdkwork-agentstudio-pc-ui/src/components/Input.tsx');
  const textareaSource = read('packages/sdkwork-agentstudio-pc-ui/src/components/Textarea.tsx');
  const switchSource = read('packages/sdkwork-agentstudio-pc-ui/src/components/Switch.tsx');
