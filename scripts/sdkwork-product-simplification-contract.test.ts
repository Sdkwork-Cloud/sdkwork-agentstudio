...
runTest('simplified product shell removes app store, ecosystem marketplaces, and commerce routes from host surfaces', () => {
  ...
  const coreSidebarSource = read('packages/sdkwork-agentstudio-pc-core/src/components/Sidebar.tsx');
  const coreCommandPaletteSource = read('packages/sdkwork-agentstudio-pc-core/src/components/CommandPalette.tsx');
...
runTest('simplified workspace checks stop requiring removed feature packages', () => {
  ...
  const structureSource = read('scripts/check-sdkwork-agentstudio-pc-structure.mjs');
  const routeSurfaceSource = read('scripts/check-sdkwork-agentstudio-pc-route-surface.mjs');
  const routeBaselineSource = read('scripts/fixtures/agent-studio-v5-route-surface.json');
  ...
  assert.doesNotMatch(structureSource, /sdkwork-agentstudio-pc-(apps|github|huggingface|mall|market|model-purchase|points)/);
  ...
  assert.doesNotMatch(featureBridgeSource, /sdkwork-agentstudio-pc-(apps|github|huggingface|model-purchase|points)/);
...
runTest('simplified workspace removes deprecated feature package directories', () => {
  const removedPackageDirs = [
    'packages/sdkwork-agentstudio-pc-apps',
    'packages/sdkwork-agentstudio-pc-github',
    'packages/sdkwork-agentstudio-pc-huggingface',
    'packages/sdkwork-agentstudio-pc-mall',
    'packages/sdkwork-agentstudio-pc-market',
    'packages/sdkwork-agentstudio-pc-model-purchase',
    'packages/sdkwork-agentstudio-pc-points',
  ];
...
runTest('simplified workspace removes obsolete product contracts and shared exports', () => {
  ...
  const coreServicesIndexSource = read('packages/sdkwork-agentstudio-pc-core/src/services/index.ts');
