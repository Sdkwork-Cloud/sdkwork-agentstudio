...
export function resolveUserCenterStandardTestFile({
  ...
  return path.join(
    ...
    'scripts',
    'user-center-standard-bridge.test.ts',
  );
}

export function resolveAuthStandardRunner({
  workspaceRoot: resolvedWorkspaceRoot = workspaceRoot,
} = {}) {
  return path.join(
    resolvedWorkspaceRoot,
    'scripts',
    'run-sdkwork-auth-check.mjs',
  );
}
...
export function createUserCenterStandardTestPlan({
  ...
  return {
    ...
    args: [
      ...
      resolveUserCenterStandardTestFile({ workspaceRoot: resolvedWorkspaceRoot }),
    ],
    cwd,
    env,
    shell: false,
...
export function createAuthStandardCommandPlan({
  workspaceRoot: resolvedWorkspaceRoot = workspaceRoot,
  cwd = resolvedWorkspaceRoot,
  env = process.env,
  nodeExecutable = process.execPath,
  ...
  return {
    command: nodeExecutable,
    args: [resolveAuthStandardRunner({ workspaceRoot: resolvedWorkspaceRoot })],
    cwd,
    env,
    shell: false,
    windowsHide: platform === 'win32',
...
export function createUserCenterStandardCommandPlan({
  ...
  const authPlan = createAuthStandardCommandPlan({
    workspaceRoot: resolvedWorkspaceRoot,
    cwd,
    env,
    nodeExecutable,
