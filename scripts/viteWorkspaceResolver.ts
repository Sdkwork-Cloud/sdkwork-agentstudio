...
const WORKTREE_WORKSPACE_PACKAGE_PATTERN = /(?:^|[\\/])\.worktrees[\\/][^\\/]+[\\/]packages[\\/](sdkwork-agentstudio-pc-[^\\/]+)([\\/].*)$/;
...
export function resolveWorkspacePackageAliases(packagesRootDir: string) {
  const localAliases = !fs.existsSync(packagesRootDir)
    ...
    .filter((entry) => entry.isDirectory() && /^sdkwork-agentstudio-pc-[^/]+$/.test(entry.name))
    .map((entry) => ({
