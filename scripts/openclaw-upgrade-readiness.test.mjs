...
await runTest('assessOpenClawUpgradeReadiness reports ready when local upgrade inputs already exist', async () => {
  ...
  try {
    ...
    createJson(path.join(tempRoot, 'packages', 'sdkwork-agentstudio-pc-desktop', 'src-tauri', 'resources', 'openclaw', 'manifest.json'), {
      schemaVersion: 1,
      runtimeId: 'openclaw',
    ...
    createJson(path.join(tempRoot, 'packages', 'sdkwork-agentstudio-pc-desktop', 'src-tauri', 'resources', 'openclaw', 'runtime', 'package', 'node_modules', 'openclaw', 'package.json'), {
      name: 'openclaw',
...
await runTest(
  ...
  async () => {
    ...
    try {
      ...
      assert.deepEqual(result.blockers, [
        `Unsupported OpenClaw runtime layout is present at packages/sdkwork-agentstudio-pc-desktop/src-tauri/resources/openclaw-runtime (detected version ${retiredOpenClawVersion}). Remove retired layout artifacts before upgrading the packaged OpenClaw runtime.`,
      ]);
    } finally {
      rmSync(tempRoot, { recursive: true, force: true });
    }
