...
await runTest('sdkwork-agentstudio-pc-core node root entry loads under Node without browser-only runtime wiring', async () => {
  const moduleUrl = pathToFileURL(path.join(process.cwd(), 'packages/sdkwork-agentstudio-pc-core/src/node.ts')).href;
