...
describe('createAppSdkClientConfig', () => {
  ...
  it('persists auth runtime state into standardized core and user-center storage keys', () => {
    ...
    expect(globalThis.sessionStorage.getItem('agent-studio.user-center.auth-token')).toBe('auth-token');
    expect(globalThis.sessionStorage.getItem('agent-studio.user-center.access-token')).toBe('owner-access-token');
    expect(globalThis.sessionStorage.getItem('agent-studio.user-center.refresh-token')).toBe('refresh-token');
    expect(globalThis.localStorage.getItem('agent-studio-auth-session')).toBeNull();
  });

  it('reads auth runtime state back from standardized user-center storage when runtime storage is empty', () => {
    globalThis.sessionStorage.setItem('agent-studio.user-center.auth-token', 'user-center-auth');
    globalThis.sessionStorage.setItem('agent-studio.user-center.access-token', 'user-center-access');
    globalThis.sessionStorage.setItem('agent-studio.user-center.refresh-token', 'user-center-refresh');
