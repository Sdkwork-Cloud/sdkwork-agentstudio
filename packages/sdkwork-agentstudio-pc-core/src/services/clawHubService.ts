...
import type { Review, Skill, SkillPack } from '@sdkwork/agentstudio-pc-types';
import { unwrapAppSdkResponse } from '../sdk/appSdkResult.ts';
import {
  getagentstudioAppClientWithSession,
...
type ClawHubClient = { skill: import('../sdk/appSdkPort.ts').agentstudioSkillClient };
...
function getDefaultClient(): ClawHubClient {
  return getagentstudioAppClientWithSession() as ClawHubClient;
}
...
async function fetchAllSkillPages(
...
export function createClawHubService(
  ...
  return {
    ...
    async listSkills(params = {}) {
      ...
      const client = getClient ? getClient() : getDefaultClient();
      return (await fetchAllSkillPages(client, params)).map(mapSkill);
    },
