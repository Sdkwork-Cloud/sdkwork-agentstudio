...
pub const CLAW_SERVER_DEFAULT_DATA_DIR: &str = ".agentstudio-server";
pub const CLAW_SERVER_DEFAULT_WEB_DIST_DIR: &str = "../sdkwork-agentstudio-pc-web/dist";
pub const CLAW_SERVER_DEFAULT_CONFIG_FILE_NAME: &str = "agentstudio-server.config.json";
...
fn resolve_packaged_server_bundle_paths(
    ...
    let executable_name = executable_path
        ...
        .trim()
        .to_ascii_lowercase();
    if executable_name != "agentstudio-server" && executable_name != "agentstudio-server.exe" {
        return None;
    }
...
mod tests {
    ...
    fn runtime_config_uses_packaged_bundle_defaults_when_running_bundled_binary() {
        ...
        let executable_path = bundle_root.join("bin").join("agentstudio-server");
        let web_dist_dir = bundle_root.join("web").join("dist");
        ...
        assert_eq!(resolved.data_dir, bundle_root.join(".agentstudio-server"));
    ...
    fn runtime_config_ignores_packaged_shape_without_bundled_web_dist() {
        ...
        assert_eq!(resolved.data_dir, PathBuf::from(".agentstudio-server"));
        assert_eq!(
            resolved.web_dist_dir,
    ...
    fn unique_missing_config_path(label: &str) -> PathBuf {
        ...
        env::temp_dir().join(format!(
            "sdkwork-agentstudio-standalone-gateway-{label}-{}-{unique_suffix}.json",
            std::process::id()
        ))
    }
    ...
    fn unique_bundle_root(label: &str) -> PathBuf {
        let unique_suffix = SystemTime::now()
            ...
            .as_nanos();
        env::temp_dir().join(format!(
            "sdkwork-agentstudio-standalone-gateway-bundle-{label}-{}-{unique_suffix}",
            std::process::id()
        ))
    }
}
