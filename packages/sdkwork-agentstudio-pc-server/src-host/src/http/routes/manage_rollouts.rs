...
use sdkwork_agentstudio_host_core::domain::rollout::ManageRolloutTargetPreviewRecord;
use sdkwork_agentstudio_host_core::internal::error::{InternalErrorCategory, InternalErrorResolution};
use sdkwork_agentstudio_host_core::rollout::control_plane::{
...
async fn list_rollouts(
    ...
    let params = parse_offset_list_params(&raw_query)?;
    state
        .rollout_control_plane
        .list_rollouts()
        .map(|data| {
            let total = data.items.len() as i64;
            let page = offset_list_page_data(data.items, total, params);
...
fn parse_offset_list_params(raw_query: &str) -> Result<OffsetListPageParams, Response> {
