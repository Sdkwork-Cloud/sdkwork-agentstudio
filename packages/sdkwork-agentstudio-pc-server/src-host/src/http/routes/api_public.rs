...
use sdkwork_utils_rust::http_api::{
    offset_list_page_data, offset_limit_page_from_iter,
    validated_offset_list_params, OffsetListPageParams, SdkWorkCursorListQuery,
};
...
async fn list_public_studio_instances(
    ...
    let params = parse_offset_list_params(&raw_query, &trace_id)?;
    ...
    .map(|data| {
        ...
        let page = offset_list_page_data(items, total, params);
...
async fn load_public_studio_kernel_chat_messages(
    ...
    authorize_public_studio_request(&headers, &state, &trace_id)?;
    let provider = require_studio_public_api_provider(&state)?;
    let query = parse_cursor_list_query(&raw_query, &trace_id)?;
    let instance_id = id.clone();
    let requested_session_id = session_id.clone();
    ...
    .map(|data| {
        ...
        let page_info = sdkwork_utils_rust::http_api::cursor_window_page_info(
            Some(query.page_size),
            page.next_cursor,
            page.has_more,
        );
        api_list_success(page.items, page_info, &trace_id)
    })
    .map_err(|error| {
        studio_public_api_kernel_chat_error_response(
            &state,
...
/// Parsed offset-mode list pagination parameters.
fn parse_offset_list_params(
    raw_query: &str,
    trace_id: &str,
...
/// Parsed cursor-mode list pagination parameters.
struct CursorListQuery {
    page_size: usize,
    offset: usize,
}

fn parse_cursor_list_query(
    raw_query: &str,
    _trace_id: &str,
) -> Result<CursorListQuery, Response> {
    let query: SdkWorkCursorListQuery =
        serde_urlencoded::from_str(raw_query).unwrap_or_default();
    let params = query.resolve().map_err(|code| {
        categorized_error_response(
            "invalid_pagination_params",
            InternalErrorCategory::Validation,
            &format!("Invalid cursor pagination parameters: {}", code.symbol()),
            StatusCode::BAD_REQUEST,
            false,
            InternalErrorResolution::FixRequest,
            0,
