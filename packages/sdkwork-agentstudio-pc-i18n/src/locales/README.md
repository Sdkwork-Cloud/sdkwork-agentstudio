...
- Run `pnpm --filter @sdkwork/agentstudio-pc-i18n sync:locales` after editing split locale files so the compatibility bundles in `en.json` and `zh.json` stay in sync for legacy readers and contract tests.
