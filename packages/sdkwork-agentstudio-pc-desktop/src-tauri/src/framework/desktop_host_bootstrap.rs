...
mod tests {
    ...
    fn embedded_host_bootstrap_serves_root_html_with_desktop_combined_host_metadata() {
        ...
        assert!(body.contains("sdkwork-agentstudio-pc-host-mode"));
        assert!(body.contains("content=\"desktopCombined\""));
        assert!(body.contains("sdkwork-agentstudio-pc-api-base-path"));
        assert!(body.contains("sdkwork-agentstudio-pc-manage-base-path"));
        assert!(body.contains("sdkwork-agentstudio-pc-internal-base-path"));
        assert!(body.contains("sdkwork-agentstudio-pc-browser-session-token"));
    ...
    fn embedded_host_bootstrap_exposes_structured_browser_bootstrap_descriptor() {
        ...
        let response = tokio::runtime::Builder::new_current_thread()
            ...
            .block_on(async {
                reqwest::get(format!(
                    "{}/sdkwork-agentstudio-pc-bootstrap.json",
                    snapshot.browser_base_url
                ))
                .await
    ...
    async fn fetch_browser_session_token(
        ...
        let descriptor = client
            .get(format!("{browser_base_url}/sdkwork-agentstudio-pc-bootstrap.json"))
            .send()
            .await
            .expect("bootstrap descriptor response")
            .json::<Value>()
