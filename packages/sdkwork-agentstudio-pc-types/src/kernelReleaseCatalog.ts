...
function deriveRuntimeRequirements(
  ...
  if (!isRecord(config.runtimeRequirements)) {
    return deepClone(config.runtimeRequirements);
  }

  const runtimeRequirements = deepClone(config.runtimeRequirements);
  if (
...
function deriveReleaseSource(
  ...
  if (!isRecord(config.releaseSource)) {
    return deepClone(config.releaseSource);
  }

  const releaseSource = deepClone(config.releaseSource);
