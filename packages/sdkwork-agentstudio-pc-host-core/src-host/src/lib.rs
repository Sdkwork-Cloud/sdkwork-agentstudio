...
pub fn host_core_metadata() -> HostCoreMetadata {
    HostCoreMetadata {
        package_name: "sdkwork-agentstudio-service-host",
...
mod tests {
    ...
    fn create_test_rollout_store_path(label: &str) -> PathBuf {
        ...
        let directory = std::env::temp_dir().join(format!(
            "sdkwork-agentstudio-service-host-rollouts-{label}-{}-{unique_suffix}",
            std::process::id()
        ));
    ...
    fn create_test_catalog_sqlite_path(label: &str) -> PathBuf {
        let unique_suffix = SystemTime::now()
            ...
            .as_nanos();
        let directory = std::env::temp_dir().join(format!(
            "sdkwork-agentstudio-service-host-sqlite-{label}-{}-{unique_suffix}",
            std::process::id()
        ));
